...
with st.sidebar:
    ...
    if db.get_mode() == "cloud":
        st.markdown('<div class="db-status db-cloud">☁️ Conectado ao Supabase</div>', unsafe_allow_html=True)
    else:
        st.markdown('<div class="db-status db-local">💾 Modo Local (JSON)</div>', unsafe_allow_html=True)
    
    st.caption("Dados salvos automaticamente")
    ...
    with col_reminder_list:
        ...
        if not reminders:
            ...
            for r in sorted_reminders:
                ...
                # Definir estilo baseado no vencimento
                if diff_days < 0:
                    status = "🔴 Vencido"
                elif diff_days <= 7:
